...
&ensp; &ensp; &ensp;   $`\ket{0}_{nb} \rightarrow \ket{b}_{nb}`$  
...
$ `QPE\left(\sum_{j=0}^{N+1}  b_{j}\ket{0}_{nl}\ket{u_{j}}_{nb}\right) = \sum_{j=0}^{N+1}  b_{j}\ket{\lambda_{j}}_{nl} \ket{u_{j}}_{nb}`$
